...
- (IBAction)addNewLink:(id)sender {
    ...
    if (!link || [link rangeOfString:@"kuai"].location == NSNotFound || [link rangeOfString:@"xunlei"].location == NSNotFound) {
        NSAlert *alert = [NSAlert alertWithMessageText:NSLocalizedString(@"Please Input A Valid Kuai Link", nil) defaultButton:NSLocalizedString(@"OK", nil) alternateButton:nil otherButton:nil informativeTextWithFormat:NSLocalizedString(@"Valid Kuai Link Style: http://kuai.xunlei.com/d/XXXXXXXXXXXXXXXXXXX", nil)];
        [alert runModal];
    } else {
